...
export default function ABReport({ password }: ABReportProps) {
        ...
        <>
          <div className="grid grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <p className="text-4xl font-bold text-foreground">{stats.whatsapp_clicks}</p>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-4xl font-bold text-foreground">{stats.form_submits}</p>
          ...
          {Object.keys(stats.by_section).length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-sm font-medium">Por Seção</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {Object.entries(stats.by_section).map(([section, counts]) => (
                    <div key={section} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                      <span className="text-sm capitalize text-foreground">{section}</span>
                      <div className="flex gap-4 text-sm">
                        <span style={{ color: "hsl(142, 76%, 36%)" }}>WA: {counts.whatsapp}</span>
                        <span className="text-primary">Form: {counts.form}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {stats.by_datetime && stats.by_datetime.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-sm font-medium">Conversões por Data/Hora</CardTitle></CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {stats.by_datetime.map((row, idx) => (
                      <TableRow key={`${row.date}_${row.hour}_${row.section}_${idx}`}>
                        ...
                        <TableCell className="capitalize text-muted-foreground text-sm">{row.section}</TableCell>
                        <TableCell className="text-right" style={{ color: "hsl(142, 76%, 36%)" }}>{row.whatsapp || '-'}</TableCell>
                        <TableCell className="text-right text-primary">{row.form || '-'}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          {stats.individual_events && stats.individual_events.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-sm font-medium">Conversões Individuais</CardTitle></CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {stats.individual_events.map((event, idx) => (
                      <TableRow key={`individual_${idx}_${event.timestamp}`}>
                        ...
                        <TableCell className="capitalize text-muted-foreground text-sm">{event.section}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
